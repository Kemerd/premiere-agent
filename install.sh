#!/usr/bin/env bash
# ---------------------------------------------------------------------------
# premiere-video-editor-agent bootstrap (Linux / macOS).
#
...
echo "[premiere-video-editor-agent] python: ${PYTHON:-<none>}"
echo "[premiere-video-editor-agent] os:     ${OS_NAME} (${ARCH_NAME})"
...
if [ -n "$TORCH_INDEX" ]; then
  ...
  echo "[premiere-video-editor-agent] installing torch from PyPI default (CPU/MPS wheel for this platform)"
...
# ---------------------------------------------------------------------------
echo "[premiere-video-editor-agent] accelerator smoke test:"
"$PYTHON" - <<'PY'
import platform
import torch
