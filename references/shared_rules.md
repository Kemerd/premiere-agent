...
1. **LLM reasons from one interleaved markdown view + on-demand drill-down.** `merged_timeline.md` is the editor sub-agent's default reading surface — speech phrases, audio events, and visual captions for every source, all interleaved by timestamp in a single file. The three per-lane views (`speech_timeline.md`, `audio_timeline.md`, `visual_timeline.md`) are kept on disk as drill-down references for the moments where the merged view is ambiguous and the editor zooms in on one lane. Everything else — filler tagging, retake detection, shot classification, B-roll spotting, emphasis scoring — is derived at decision time.
...
- Reads `merged_timeline.md` end-to-end, with its own fresh context window. At vocab time the merged view contains two lanes (speech + visual) interleaved by timestamp — the audio lane is empty until Phase B runs, which is what this sub-agent's vocab enables. Drilling into `speech_timeline.md` / `visual_timeline.md` is allowed only at ambiguous moments, never as the default read.
...
- Reads `merged_timeline.md` end-to-end (per the ABSOLUTE READ MANDATE in `subagent_editor_rules.md` and the spine principle of Hard Rule 15) in its own fresh context window. Drills into the per-lane files only at ambiguous moments.
...
15. **The merged view is the spine for both the editor and the vocab sub-agents — never reason from a single lane in isolation.** Cut decisions (editor) and vocabulary curation (vocab) both read `merged_timeline.md` end-to-end so every lane visible at that step informs the output. For the editor at step 6 that means all three lanes (speech, visual, audio) interleaved — speech as the editorial spine, visuals for shot continuity / B-roll, audio events as soundscape hints. For the vocab sub-agent at step 2 that means two lanes (speech + visual; the audio lane is empty until the vocab this sub-agent is producing has been used to score it), with the alignment between adjacent timestamps doing the cross-referencing work for free. A cut chosen blind to the other lanes lands mid-shot, mid-action, or on a CLAP mis-label; a vocabulary curated blind to the visuals misses every silently-shown sound source. Drill into per-lane files only at ambiguous moments. The mandatory full-coverage read discipline for each sub-agent lives in its own rules file (`subagent_editor_rules.md` Pre-flight, `subagent_vocab_rules.md` Pre-flight) — those files are authoritative for what "read in full" means inside each role.

16. **`pack_timelines.py` runs TWICE per session — once before vocab, once after audio.** The parent's first pack (after Phase A preprocess) produces a two-lane merged timeline (speech + visual) which the vocab sub-agent reads as its single spine. The parent's second pack (after `audio_lane.py` finishes Phase B) folds the freshly-scored audio events into the same `merged_timeline.md` so the editor sub-agent in step 6 sees all three lanes on one page. Skipping the second pack ships the editor a stale merged view with no `(audio: …)` lines and silently breaks Hard Rule 15. Helpers and per-lane caches (`transcripts/`, `audio_tags/`, `visual_caps/`) are reused across both passes — the cost of the second pack is small; the cost of skipping it is editorial blindness on the audio lane.
