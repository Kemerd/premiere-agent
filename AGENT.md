...
11. **Read `merged_timeline.md` IN FULL. Treat it as life-or-death — finish the file no matter how long it takes.** The file is caveman-compressed and sentence-delta-deduped at pack time precisely so it fits comfortably in context — typical projects land in the 200KB–1.5MB range, well under any modern model's window. There is no token-budget excuse, no time-budget excuse, no "context-pressure" excuse, no "diminishing returns" excuse, no fatigue excuse. You finish the file. Period. **Forbidden behaviours**, all of which produce silently bad cuts:
    ...
    - Bailing on the remaining chunks with *"at this rate it'll consume a huge amount of context with diminishing returns"* / *"the rest is probably more of the same"* / *"I've seen enough to make the call."* You haven't. The merged view is dedup'd — every remaining line is novel signal, and the moment you cut from is statistically just as likely to live in the unread tail as the read head. This excuse is the #1 cause of silently bad cuts; recognise the urge and override it.
    - Stopping early because "the user is waiting" / "this is taking too long" / "I should get to editing." Time-to-first-EDL is not a metric. Cut quality is. A correct cut delivered in 20 minutes beats a wrong cut delivered in 5.
    - **Triggering an auto-compact / context-summarisation pass mid-read to "make room" for the rest of the file.** Compaction lossily summarises the chunks you already read — you trade verbatim timestamped lines for a paraphrase, which is exactly the partial-read failure mode wearing a different hat. If you feel the urge to compact, you are already in violation; the compressed merged view is *designed* to fit, and if a specific session is genuinely brushing the window, **stop and tell the user** ("merged_timeline.md is ~Xkb against my ~Ykb remaining budget — request: split the session, raise the budget, or explicitly authorise a compaction") rather than silently triggering it yourself. Self-compacting to dodge Hard Rule 11 is a Hard Rule 11 violation by another name.
    If the file is genuinely too large for one `Read` call (hits the per-call cap), issue *sequential* `Read` calls with `offset`/`limit` until you have covered every line — not a sample, not "most of it," every line. Treat partial-read shortcuts as a Hard Rule 11 violation regardless of how good the resulting cut looks; the user will catch it and you will be slapped.
...
|-----------|------------------------:|-------------------:|--------------:|---------------:|------|
| Calm      | 800 ms                  | 500 ms             | 500 ms        | 500 ms         | Cinematic, contemplative, breathing room. Long silences are kept; only obvious dead air is trimmed. Documentary, interview, narrative. |
| Measured  | 600 ms                  | 350 ms             | 350 ms        | 350 ms         | Conversational and considered. Professional talking-head, podcast-style, unhurried tutorial. |
| **Paced** *(default)* | **450 ms** | **200 ms**     | **200 ms**    | **200 ms**     | Balanced and modern. Retains rhythm without dragging. Default for tech demos, launch videos, mid-form content. |
| Energetic | 350 ms                  | 100 ms             | 100 ms        | 100 ms         | Tight and punchy. Social-friendly, fast tutorials, hype reels. |
| Jumpy     | 250 ms                  |  50 ms             |  50 ms        |  50 ms         | Ultra-tight "every breath cut" style — every breath, hesitation, and thinking pause dies, but fluent sentences stay intact. Montage, trailer, vlog supercuts. Risks audible artifacts on poor source audio — verify on preview. |

**Why the first column bottoms out at 250ms** (hard perceptual floor — no preset goes lower, ever): fluent connected speech carries 30–200ms articulation gaps between almost every adjacent word pair. Those gaps are not dead air — they ARE the words sounding like words. A threshold below ~250ms machine-guns a fluently spoken sentence like *"hello my name is Everett"* into five one-word clips with five audible seams, which is not "jumpy style," it's broken audio. Real editorial pauses — breaths, hesitations, thinking gaps — start around 250–300ms, which is why the ladder runs 800→250 and stops there. The presets differ in *how much pause survives* (and in their margins), never in whether individual words get glued together.

**What each number means** (apply at decision time when picking EDL ranges):

- **`min_silence_to_remove`** — silences (gaps between words from the speech lane) shorter than this are *kept*; longer ones are candidates to cut out entirely. A Jumpy preset chops out any real pause ≥250ms; a Calm preset only chops ≥800ms gaps so natural pauses survive. Gaps under the threshold — which includes every ordinary inter-word articulation gap — are never cut targets on any preset.
...
**Aggressive intra-phrase silence removal (this is the whole point of the preset).** `min_silence_to_remove` applies to **every word-to-word gap in the speech lane**, not just to phrase boundaries or speaker handoffs. If a single phrase like *"today we're going to ⟨640ms gap⟩ drill the pilot holes"* contains a gap ≥ the threshold, you MUST split that phrase into two adjacent ranges from the same source — `[..., "going", "to"]` then `[..., "drill", "the", "pilot", "holes"]` — so the dead air is dropped from the timeline. This is how a Paced preset turns a 12-minute walking-talking-head into 7 minutes without losing a word: by deleting hundreds of breath gaps, hesitations, and thinking pauses scattered inside otherwise-kept speech. Do not romanticize the "natural rhythm of how someone talks" — the preset *is* the rhythm decision. If the user wants those pauses kept, they pick Calm.

**The flip side — never split fluent speech.** "Every gap" means every gap *at or above the threshold*, and the threshold floor (250ms on Jumpy) sits deliberately above ordinary inter-word articulation gaps. A fluently delivered sentence with no gap ≥ the threshold exports as ONE uninterrupted range — even on Jumpy. If your draft EDL shows several cuts landing inside a single spoken sentence, that is not "the jumpy aesthetic," it's a misapplied threshold: the seams will click and the delivery will stutter. Jumpy earns its name by killing every *pause* between phrases and sentences, not by slicing between words.
...
- **Visual context is the second source of truth.** Before committing to *any* non-trivial cut, check the `visual:` lines around the cut point in `merged_timeline.md`. If captions show a continuous action ("person holding drill") spanning your cut, you're cutting in the middle of a shot — usually fine, but be deliberate. Use the visual lane to find B-roll cutaway candidates, match cuts, shot changes, and to decide whether a moment is worth preserving even when speech is silent. Drill into `visual_timeline.md` when you need the full 1-fps caption stream (the merged view drops `(same)` repeats, and on visually dense projects auto-thins to every other caption to stay scannable — a banner at the top of `merged_timeline.md` calls this out; `visual_timeline.md` is always full density, so when you need precise shot continuity near a cut, B-roll candidate scoping, or any moment where the merged visuals feel sparse, drill there).
...
- **Silence gaps are cut candidates — EVERYWHERE, not just at phrase boundaries.** Use the pacing preset's `min_silence_to_remove` as your threshold (Calm 800ms → Jumpy 250ms) and apply it to every adjacent word pair in the speech lane, including gaps that sit *inside* a phrase as the speaker pauses to breathe or think. Splitting a phrase mid-sentence to drop a 400ms thinking pause is the whole point of the preset; it's how you cut runtime without cutting content. The user picked Energetic because they want every breath gone — give them every breath. But a breath is a pause, not a word gap: anything shorter than the preset threshold — including every ordinary 30–200ms inter-word articulation gap — stays as the natural rhythm of the speech, on every preset. A fluent sentence never gets split just because words have air between them. (<30ms is always unsafe — mid-phoneme.)
...
```json
{
  ...
  "pacing_preset": "Paced",
  "pacing": {"min_silence_to_remove_ms": 450,
...
- **"Saving tokens" by partial-reading `merged_timeline.md`.** First-N-lines, last-N-lines, "representative sample," grep-and-edit-from-matches, abandoning a chunked read because "I have enough," *"at this rate it'll consume a huge amount of context with diminishing returns,"* "the rest is probably more of the same," "I should stop and get to editing so the user isn't waiting," delegating the full read to another agent to "protect context," **silently triggering an auto-compact / context-summarisation pass mid-read to free up room** — all forbidden (Hard Rule 11). Self-compacting is the same partial-read failure with extra steps: you replace verbatim timestamped lines with a lossy paraphrase, then edit from the paraphrase. The file is compressed and dedup'd at pack time so every remaining line is novel signal; if it exceeds one `Read` call, issue sequential `Read` calls with `offset`/`limit` until every line is covered. If the session genuinely cannot fit the file, **stop and tell the user** — never silently compact your own history to dodge the rule. Treat the full read as life-or-death — finish it no matter how long it takes. YOU make the taste calls; outsourcing the read, stopping early, or compressing your own context outsources the judgement.
...
- **Machine-gunning fluent speech.** Splitting inside a fluently delivered sentence because ordinary 30–200ms inter-word articulation gaps got treated as cut targets — *"hello ⟨cut⟩ my name ⟨cut⟩ is Everett"* is five clips of nothing gained and five audible seams. The preset ladder's `min_silence_to_remove` floor is 250ms (Jumpy) precisely so no preset ever slices between words that were spoken as one breath. Presets remove PAUSES — breaths, hesitations, thinking gaps ≥ the threshold. If a sentence contains no gap ≥ the preset's threshold, it exports as one uninterrupted range, even on Jumpy. Multiple cuts landing inside one spoken sentence in your draft EDL is a red flag to re-check the gap math, not a style.
