...
- If `merged_timeline.md` would exceed 888 KB, `pack_timelines.py` applies one extra visual-only squeeze: vowels inside visual `[...]` words are dropped unless they occupy the first or last character slot (`absorber -> absrbr`, `toolbox -> tlbx`). Speech and audio lines are untouched. If a squeezed visual phrase is unclear, reference the same timestamp in `visual_timeline.md` before deciding.
