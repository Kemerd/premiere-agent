...
- **"Saving tokens" by partial-reading `merged_timeline.md`.** First-N-lines, last-N-lines, "representative sample," grep-and-edit-from-matches, abandoning a chunked read because "I have enough," *"at this rate it'll consume a huge amount of context with diminishing returns,"* "the rest is probably more of the same," "I should stop and get to editing so the user isn't waiting," delegating the full read to another agent to "protect context" — all forbidden (Hard Rule 11). The file is compressed and dedup'd at pack time so every remaining line is novel signal; if it exceeds one `Read` call, issue sequential `Read` calls with `offset`/`limit` until every line is covered. Treat the full read as life-or-death — finish it no matter how long it takes. YOU make the taste calls; outsourcing the read or stopping early outsources the judgement.
